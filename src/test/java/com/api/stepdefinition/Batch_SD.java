...
public class Batch_SD extends BaseTest {
	...
	public void user_creates_request_for_the_lms_api_endpoint(String jsonKey) throws FileNotFoundException {
		...
		data.put("batchName", "API_testers-799");
		data.put("batchNoOfClasses", 20);
		data.put("batchStatus", "Active");

//		.body(JsonReader.getRequestBody(jsonFileName, jsonKey)) 
		request = given().spec(CommonSpec()).body(data.toString()).header("Authorization",
				"Bearer " + PC_token.getToken());
	...
	public void user_sends_https_post_request_for_batch_and_request_body_with_mandatory_and_additional_fields() {
		...
		response = request.when().post(API_Path.apiPath.createBatch).then().log().all().extract().response();
	...
	public void user_receives_created_status_for_batch(Integer statusCode) {
		...
		response.then().assertThat().statusCode(statusCode).statusLine("HTTP/1.1 201 ")
...
//		
	...
	@When("Admin sends HTTPS Request with batch endpoint")
	public void admin_sends_https_request_with_batch_endpoint() {
		response = request.when().post(API_Path.apiPath.createBatch).then().log().all().extract().response();
		batchID = response.path("batchId");
		programID = response.path("programId");
	...
	public void admin_receives_bad_request_status_with_message_and_boolean_success_details(Integer statusCode) {
		...
		response.then().assertThat().statusCode(statusCode).statusLine("HTTP/1.1 400 ").header("Content-Type",
				"application/json");
	...
	public void admin_creates_post_request_with_invalid_data_in_request_body(String jsonKey)
			throws FileNotFoundException {
	...
	public void admin_sends_https_request_with_invalid_endpoint() {
		response = request.when().post(API_Path.apiPath.invalidcreateBatch).then().log().all().extract().response();
	}

	@Then("Admin receives {int} not found  Status")
	public void admin_receives_not_found_status(Integer statusCode) {
		// Validating Status Code,Status Line,Header
		response.then().assertThat().statusCode(statusCode).statusLine("HTTP/1.1 404 ").header("Content-Type",
				"application/json");
...
//			.body(JsonReader.getRequestBody(jsonFileName, jsonKey))
		data = new JSONObject();
		data.put("programId", pojo.getProgramId());
		data.put("batchName", "API_testers-489");
		data.put("batchNoOfClasses", 20);
		data.put("batchStatus", "Active");
		request = given().spec(CommonSpec()).body(data.toString()).header("Authorization",
				"Bearer " + PC_token.getToken());
	...
	@Given("Admin creates POST Request with invalid data in request body {string}")
	public void admin_creates_post_request_with_invalid_data_in_request_body1(String jsonKey)
			throws FileNotFoundException {
		request = given().spec(CommonSpec()).body(JsonReader.getRequestBody(jsonFileName, jsonKey))
				.header("Authorization", "Bearer " + PC_token.getToken());
	...
	public void admin_receives_unauthorized(Integer statusCode) {
		// Validating Status Code,Status Line,Header,Json Schema
		response.then().assertThat().statusCode(statusCode).statusLine("HTTP/1.1 401 ").header("Content-Type",
				"application/json");
	}
	...
	public void admin_sends_with_endpoint(String get) {
		...
		response = request.when().get(API_Path.apiPath.getAllBatches);
	...
	public void admin_receives_ok_status_with_response_body(Integer statusCode) {
		// Validating Status Code,Status Line,Header,Json Schema
		response.then().assertThat().statusCode(statusCode).statusLine("HTTP/1.1 200 ").header("Content-Type",
				"application/json");
	...
	public void admin_sends_request_with_endpoint(String string) {
		response = request.when().get(API_Path.apiPath.getBatchByBatchID + batchID).then().log().all().extract()
				.response();
...
//@GetBatchByBatchId
	...
	public void admin_sends_https_get_request_with_endpoint() {
		...
		response = request.when().log().all().get(API_Path.apiPath.getBatchByBatchID + batchID).then().log().all()
				.extract().response();
	...
	public void admin_receives_ok_status_with_response_body_for_batch(Integer statusCode) {
		...
		// Validating Status Code,Status Line,Header,Json Schema
		response.then().assertThat().statusCode(statusCode).statusLine("HTTP/1.1 200 ")
				.header("Content-Type", "application/json")
	...
	public void admin_sends_https_request_with_endpoint(String string) {
		response = request.when().get(API_Path.apiPath.getBatchByBatchID + 85220).then().log().all().extract()
				.response();
	}
	...
	public void admin_sends_https_request_with(String endpoint) {
		response = request.when().get(API_Path.apiPath.invalidEndpoint + endpoint).then().log().all().extract()
				.response();
	}
	...
	public void admin_sends_https_request_with_batch_name_endpoint() {
		...
		response = request.when().get(API_Path.apiPath.getBatchByBatchName + batchName).then().log().all().extract()
				.response();
	...
	public void admin_sends_https_request_with_endpoint_invalid_batch_name() {
		response = request.when().get(API_Path.apiPath.getBatchByBatchName + API_Path.apiPath.invalidBatchName).then()
	...
	public void admin_sends_https_request_with_invalid_endpoint_batch_name() {
		...
		response = request.when().get(API_Path.apiPath.invalidEndpoint + batchName).then().log().all().extract()
				.response();
	}
	...
	public void admin_sends_https_request_with_endpoint_for_program_id() {
		...
		response = request.when().get(API_Path.apiPath.getBatchByProgramID + programID).then().log().all().extract()
				.response();
	...
	public void admin_sends_https_request_with_invalid_program_id_endpoint() {
		...
		response = request.when().get(API_Path.apiPath.getBatchByProgramID + 768657688).then().log().all().extract()
				.response();
	...
	public void admin_sends_https_request_with_invalid_endpoint_for_get_batch() {
		response = request.when().get(API_Path.apiPath.invalidEndpoint + programID).then().log().all().extract()
				.response();
	}
	...
	public void admin_sends_https_request_with_update_endpoint() {
		...
		batchID = batch1.getBatchId();
		response = request.when().put(API_Path.apiPath.updateBatch + 8466).then().log().all().extract().response();
	...
	public void admin_creates_put_request_with_auth_valid_batch_id_and_Update_Valid_Data()
			throws FileNotFoundException {
		System.out.println(programID);
		data1 = new JSONObject();
		data1.put("batchDescription", "SDET BatchUpdates");
		data1.put("batchName", "API_testers-658");
		data1.put("batchNoOfClasses", 20);
		data1.put("batchStatus", "Active");
		data1.put("programId", programID);

//		.body(JsonReader.getRequestBody(jsonFileName, jsonKey))
		request = given().spec(CommonSpec()).body(data1.toString()).header("Authorization",
				"Bearer " + PC_token.getToken());
	}
	...
	public void admin_sends_https_request_with_endpoint_invalid_batch() {
		response = request.when().put(API_Path.apiPath.updateBatch + 846678888).then().log().all().extract().response();
	}
	...
	public void admin_sends_https_put_request_with_invalid_endpoint() {
		response = request.when().put(API_Path.apiPath.invalidEndpoint + 8466).then().log().all().extract().response();
	...
	public void admin_creates_put_request_with_valid_batch_id_and_deleted_program_id() throws FileNotFoundException {
		request = given().spec(CommonSpec())
				.body(JsonReader.getRequestBody(jsonFileName, "Update with deleted programId"))
				.header("Authorization", "Bearer " + PC_token.getToken());
	...
	public void admin_sends_https_request_with_update_endpoint_and_deleted_program_id() {
		response = request.when().post(API_Path.apiPath.updateBatch + 8452).then().log().all().extract().response();
	...
	public void admin_sends_https_request_with_endpoint_for_batch() {
		response = request.when().delete(API_Path.apiPath.deleteBatchByBatchId + batchID).then().log().all().extract()
				.response();
	}
	...
	public void admin_sends_https_request_with_invalid_delete_batch_endpoint() {
		response = request.when().delete(API_Path.apiPath.invalidEndpoint).then().log().all().extract().response();
	}
	...
	public void admin_sends_https_request_with_endpoint_invalid_batch_id() {
		response = request.when().delete(API_Path.apiPath.deleteBatchByBatchId + 988888).then().log().all().extract()
				.response();
	}
	...
	public void admin_receives_ok_status_with_batch_status_field_in_the_response_body(Integer statusCode,
			String string) {
	...
	public void admin_sends_https_request_with_endpoint_and_deleted_batch_id() {
		response = request.when().delete(API_Path.apiPath.deleteBatchByBatchId + 8466).then().log().all().extract()
				.response();
	...
	//** 
//	@When("Admin sends HTTPS Request with the endpoint for programId.")
//	public void admin_sends_https_request_with_the_endpoint_for_program_id() {
//		programID=batch1.getProgramId();
...
//	}
	...
	public void admin_creates_get_request_with_program_id_for_batch() throws FileNotFoundException {
		request = given().spec(CommonSpec())
                .header("Authorization", "Bearer " + PC_token.getToken());
	}
	@Given("User creates request for LMS API endpoint for batch {string}.")
	public void user_creates_request_for_lms_api_endpoint_for_batch(String jsonKey) throws FileNotFoundException {
		request = given().spec(CommonSpec()).body(JsonReader.getRequestBody(jsonFileName, jsonKey))
				.header("Authorization", "Bearer " + PC_token.getToken());
